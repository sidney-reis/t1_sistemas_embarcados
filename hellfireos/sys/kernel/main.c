...
static void poolingServerTask(void)
{
    for(;;)
    {
        kprintf("\nPOOLING SERVER\n");
...
static void aperiodicTaskGenerator(void)
{
    for(;;)
    {
        kprintf("\nGENERATING APERIODIC TASK\n");
        hf_spawn(dummyTask, 0, 5, 0, "dummy task", 1024);
        wait(20);
    }
}
...
int main(void)
{
	...
	if (oops == 0xbaadd00d){
        ...
        hf_spawn(poolingServerTask, 20, 2, 20, "pooling server", 1024);
        hf_spawn(aperiodicTaskGenerator, 50, 2, 50, "pooling server", 1024);
